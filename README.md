...
│
├── urdf/
│   ├── soccerbot_blue.urdf        # Blue autonomous robot model
│   └── soccerbot_red.urdf         # Red teleoperated robot model
│
├── worlds/
│   └── soccer_field.world         # Gazebo SDF world: field, walls, goals, ball
│
├── launch/
│   ├── soccer_sim.launch          # Main launch file (both robots + field)
│   └── teleop.launch              # Optional standalone teleop launch
│
├── scripts/
│   ├── ball_follower.py           # Autonomous vision + proportional control node (blue)
│   ├── teleop_blue.py             # WASD teleoperation node (blue robot)
│   └── teleop_red.py              # Arrow key teleoperation node (red robot)
│
├── config/
│   └── control_params.yaml        # Tunable control parameters (gains, thresholds)
│
├── docs/
│   ├── Project_Report.pdf         # Full course project report
│   ├── abstract_proposal.pdf      # Original project proposal
